...
public static class HyperSpinThemeLoader
{
	...
	private static HyperSpinThemeDefinition LoadFromDirectory(string absoluteThemePath, string? videoOverridePath)
	{
		...
		return BuildTheme(
			...
			relativePath => TryLoadDirectoryImage(assetRoot, relativePath),
	...
	private static HyperSpinThemeDefinition LoadFromZip(string zipPath, string? videoOverridePath)
	{
		...
		return BuildTheme(
			...
			relativePath => TryLoadZipImage(archive, CombineZipPath(xmlDirectory, relativePath)),
	...
	private static HyperSpinThemeDefinition BuildTheme(
		...
		Func<string, Texture2D?> tryLoadOptionalImage,
		Func<string?> resolveVideoPath)
	{
		...
		foreach (XElement element in root.Elements())
		{
			...
			{
				Texture2D? videoOverlay = ShouldTryVideoOverlay(element)
					? tryLoadOptionalImage("Video.png")
					: null;
				theme.Video = ParseVideo(element, resolveVideoPath(), videoOverlay, theme.Warnings);
				continue;
			}
	...
	private static Texture2D? TryLoadDirectoryImage(string assetRoot, string relativePath)
	{
		string absolutePath = Path.Combine(assetRoot, relativePath);
		return File.Exists(absolutePath) ? Utils.LoadExternalImage(absolutePath) : null;
	}

	private static Texture2D? TryLoadZipImage(ZipArchive archive, string entryPath)
	{
		return FindEntryCaseInsensitive(archive, entryPath) == null
			? null
			: LoadZipImage(archive, entryPath);
	}
	...
	private static bool ShouldTryVideoOverlay(XElement element)
	{
		return element.Attributes().Any(attribute =>
			string.Equals(attribute.Name.LocalName, "overlaybelow", StringComparison.OrdinalIgnoreCase)
			|| string.Equals(attribute.Name.LocalName, "overlayoffsetx", StringComparison.OrdinalIgnoreCase)
			|| string.Equals(attribute.Name.LocalName, "overlayoffsety", StringComparison.OrdinalIgnoreCase));
	}
